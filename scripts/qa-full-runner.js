...
const DEFAULT_CASES_PATHS = [
  ...
  'test_input.tsv',
  'test_input.json',
  path.join('testset', 'golden-set-v1.json'),
  path.join('testset', 'cases.json'),
];

const SUPPORTED_EXTS = new Set(['.xlsx', '.xls', '.csv', '.tsv', '.json']);
...
function resolveDefaultCaseSources() {
  return DEFAULT_CASES_PATHS
    .map((raw) => path.resolve(process.cwd(), raw))
    .filter((p) => fs.existsSync(p));
}

function resolveCasePath(raw) {
  const candidate = path.isAbsolute(raw) ? raw : path.join(process.cwd(), raw);
  if (fs.existsSync(candidate)) {
    const stat = fs.statSync(candidate);
    if (stat.isDirectory() || path.extname(candidate)) return candidate;
  }

  const ext = path.extname(candidate);
  if (ext) return candidate;

  const extensions = Array.from(SUPPORTED_EXTS);
...
function collectCaseSources(rawInputs) {
  const inputs = rawInputs.length ? rawInputs : resolveDefaultCaseSources();
  const list = [];
  const seen = new Set();
  ...
  for (const raw of inputs) {
    const source = resolveCasePath(raw);
...
function loadCasesFromSource(filePath, seenIds, stats) {
  ...
  const parser = ext === '.json' || ext === '' ? parseJSONFile : parseSpreadsheetFile;
...
async function main() {
  ...
  If --cases is omitted, the runner tries:
  - ./test_input (auto-detect .json/.xlsx/.xls/.csv/.tsv)
  - ./testset/golden-set-v1.json
  - ./testset/cases.json
