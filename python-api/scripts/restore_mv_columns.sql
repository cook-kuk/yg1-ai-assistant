-- Restore 3 columns dropped during a recent MV refresh:
--   holemaking_point_angle  (37k non-empty rows)
--   threading_pitch         (28k non-empty rows)
--   threading_tpi           (25k non-empty rows)
--
-- Generated: review + run manually. PG doesn't support
-- CREATE OR REPLACE for materialized views, so DROP + CREATE
-- is the only path. Indexes are recreated at the end.
...
-- Re-create indexes captured at export time.
...
REFRESH MATERIALIZED VIEW catalog_app.product_recommendation_mv;
