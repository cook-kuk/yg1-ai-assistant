...
export function ManualDownloadButton() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-50 w-72 rounded-lg border border-blue-200 bg-white shadow-xl p-1">
          <div className="px-3 py-2 text-[10px] text-gray-500 border-b border-gray-100">
            사용법 + 112 CNC 용어 사전 · Sandvik/Harvey 출처 포함
          </div>
          {DOWNLOADS.map(d => (
            <a key={d.href} href={d.href} download
              className="flex items-center gap-2 px-3 py-2 text-xs text-gray-700 hover:bg-blue-50 rounded">
              <d.icon className="h-4 w-4 text-blue-600 flex-shrink-0" />
              <span className="flex-1">{d.label}</span>
              <span className="text-[10px] text-gray-400 font-mono">{d.kb}KB</span>
            </a>
          ))}
        </div>
      )}
