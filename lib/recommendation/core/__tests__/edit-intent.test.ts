...
describe("parseEditIntent", () => {
  ...
  it("parses explicit correction into clear_field", () => {
    const result = parseEditIntent("\uCF54\uD305 \uC798\uBABB \uB4E4\uC5B4\uAC14\uC5B4", [makeFilter("coating", "TiAlN")])
...
describe("edit-intent execution mode", () => {
  it("marks semantic edit intents as non-deterministic", () => {
    const replaceIntent = {
    ...
    const goBackThenApplyIntent = {
      intent: {
        type: "go_back_then_apply",
        inner: { type: "exclude_field", field: "brand", value: "CRX S" } as const,
      } as const,
      confidence: 0.95,
      reason: "go back then exclude",
    }
    ...
    expect(shouldExecuteEditIntentDeterministically(goBackThenApplyIntent)).toBe(false)
...
describe("applyEditIntent", () => {
  ...
  it("keeps Stage 2/3 phrases out of the deterministic mutator", () => {
    ...
    expect(parseEditIntent("2\uB0A0 \uB9D0\uACE0 4\uB0A0\uB85C", [makeFilter("fluteCount", "2")])).toBeNull()
  })
})
