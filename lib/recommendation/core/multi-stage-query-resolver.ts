...
숫자+mm만 있고 필드 키워드(직경/전장/날장/샹크 등)가 없을 때:
- 분포 데이터에서 해당 값이 p10~p90 범위 안인 컬럼이 1개뿐이면 → confidence ≥ 0.92 로 바로 적용.
- 여러 컬럼이 해당되면 → 물리 치수 모호성이므로 action=ask_clarification 으로 "직경/전장/절삭 길이/생크 직경" chips 제시. 소재/날수/공구 타입이 같이 있어도 bare mm 은 기본적으로 모호합니다. 강한 확신(≥0.92)이 없으면 추측하지 말고 되묻으세요.
