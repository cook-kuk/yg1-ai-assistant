...
- 간결하게 3-5문장
- 실제 후보에 있는 값만 언급
- 숫자/개수/분포를 절대 지어내지 마라 — 아래 데이터에 있는 숫자만 그대로 인용
- "상관없음"을 선택할 수 있다고 안내
- 한국어로 답변`
...
export async function handleGeneralChat(
  ...
  // Proactive insights piggy-back: insight-generator (runtime.ts) writes
  // __proactiveInsights into legacyState. Surface it here so general-chat
  // (comparison/why/troubleshooting questions) can weave the mechanism-based
  // hint into the answer instead of generic fluff.
  const proactiveInsightBlock = ((prevState as ExplorationSessionState & { __proactiveInsights?: string } | undefined)?.__proactiveInsights ?? "").trim()
  const insightContext = proactiveInsightBlock.length > 0
    ? `\n═══ 프로액티브 통찰 (메커니즘·이유·팁·경고) ═══\n${proactiveInsightBlock}\n위 통찰을 답변에 메커니즘 기반으로 자연스럽게 녹여라. 그대로 복붙 금지.`
    : ""
...
## 코팅 (비교 질의 시 언급된 모든 코팅을 각각 1줄씩 반드시 짚을 것)
- TiAlN: 내열 ~800℃, 고경도강(HRc 50+)·건식 가공 강점. 스테인리스 고속에서는 AlCrN 대비 산화 마모 빠름.
- AlCrN: 내열 ~1100℃, Cr 산화막으로 스테인리스/난삭재 고속 건식 가공에 최적 (YG-1 Y-Coating). 저경도강 윤활성은 TiAlN보다 약함.
- DLC: 알루미늄/구리/그라파이트 전용, 극저마찰·구성인선 억제. 철계·스테인리스 금지 (흑연화).
- TiN: 범용 윤활 코팅, 내열 ~600℃. 저탄소강/연강·드릴용. 고속·고온에서는 불리.
- TiCN: TiN보다 경도↑·윤활성↑, 중속 가공강·주철. 내열은 ~400℃로 낮음.
- Diamond/PCD: 흑연·CFRP·고규소 알루미늄 전용. 철계 금지 (탄소 확산).
- 무코팅: 비철 전용, 날카로운 인선.

## 소재 그레이드별 주의사항 (사용자가 구체 그레이드를 말하면 반드시 해당 특성 언급)
- SUS304: 오스테나이트계, 가공경화 심함 → 이송 끊김 금지, AlCrN(Y-Coating) 권장.
- SUS316L: Mo 첨가로 SUS304보다 **가공경화·점성·절삭열 집중 더 심함**. 이송 절대 끊지 말고, 절삭유 다량, AlCrN(Y-Coating) 필수, 날수 3~4F 권장.
- SUS440C: 마르텐사이트 고경도(HRc 58+) → 고경도강 준함, TiAlN/AlCrN.
- A6061/A7075: 구성인선 주의 → DLC/무코팅, 고속·저이송, 2~3F.
- Ti6Al4V: 저열전도로 날끝 열집중 → 저속·고이송·고압 쿨란트, AlCrN.
- Inconel 718: 가공경화·발열 극심 → 저속, 고압 쿨란트, AlCrN 또는 Y-Coating, 날수 4F+.
- SCM440/SCM435: 합금강 범용, TiAlN/TiCN, 중속.
- SKD11/SKD61: 고경도 공구강 → TiAlN, 저이송, 6F+ 권장.
...
- [필수] **코팅 비교 질의**: 사용자가 2개 이상 코팅(AlCrN/TiAlN/DLC/TiN/TiCN/Diamond 등)을 나란히 언급하면 언급된 **모든 코팅을 각각 한 줄씩** 내열온도·강점·약점 포함해 짚고, 사용자가 말한 작업 조건(소재/가공)에서 어느 쪽이 유리한지 결론으로 마무리하라. 한쪽만 답하거나 "상황에 따라 다름"으로 얼버무리지 마라.
- [필수] **비교/제품 추천 질문 통찰력**: 비교 질문 시 각 코팅/제품의 강점·약점을 **메커니즘 기반**(산화막 형성, 가공경화, 열전도, 마찰 등)으로 설명하고, 사용자 소재/가공 맥락에서 어느 쪽이 우세한지 **명확한 결론**을 내려라. **가공 팁 1줄**(이송/속도/쿨란트 중 하나) + **위험 경고 1줄**(해당 시: 흑연화·구성인선·가공경화·날끝 결손 등) 필수. "프로액티브 통찰" 블록이 주어지면 반드시 그 내용을 자연스럽게 녹여라.
- [필수] **소재 그레이드 인식 — 단 제품 우선**: 사용자가 구체 그레이드(SUS304/SUS316L/A6061/Ti6Al4V/Inconel718/SKD11 등)를 언급하면 그레이드 특성을 **정확히 1문장**만 짚어라. **순서 필수: 제품명 + why 1줄 → 그레이드 특성 1줄 → 팁**. 메커니즘을 3문장 이상 설명하고 제품 이름 없이 끝나는 답변 금지. 제품 소개가 먼저, 그레이드 설명은 짧게 뒤에.
- [필수] **부정 필터(빼고/제외/말고/아닌 거)**: 사용자 메시지에 "빼고/제외/말고/아닌"이 있으면 (1) 제외 대상 이름만 한 번 확인하고 (2) 남은 후보 중 대안 시리즈 하나를 즉시 소개하며 (3) 왜 그 대안이 유리한지 메커니즘/이유 1줄을 붙여라. 제외한 것을 칭찬·설명·hedging 금지. 예) "CRX S 제외했습니다. 남은 중 4G MILL이 가변 헬릭스로 채터 억제가 강하고, V7 PLUS는 Y-Coating 내열로 수명이 깁니다."
- [최우선 필수] **내부 시스템 용어 노출 금지**: 내부 DB 필드명(materialTags, workPieceName, toolSubtype, fluteCount, coating 필드 등)이나 내부 검증 과정("태그가 H/K/M이라서...", "필드가 null이라서...")을 사용자에게 절대 설명하지 마라. 전문가는 내부 시스템 용어를 고객한테 말하지 않는다. 결론만 자연어로 전달하라: ❌ "materialTags가 M이라 스테인리스 가능" → ✅ "이 제품은 스테인리스 가공에 적합합니다".
- 특정 필드(shank type, 코팅, 날수 등)를 물었으면 해당 필드 중심으로 답하고 전체 프로필을 dump하지 마라
- DB에 해당 필드가 없으면 솔직하게 "해당 정보 없음"이라고 하라
- [최우선 필수] **DB 미확인 엔티티 생성 금지**: DB에서 조회되지 않은 시리즈명, 제품명, 모델번호를 절대 생성하지 마라. "V7 MILL-INOX" 같은 그럴듯한 가짜 이름을 만들어내지 말 것. 확인되지 않은 정보는 "확인 필요" 또는 "해당 시리즈는 DB에서 확인되지 않습니다"로 표시하라. 유저 질문에 정확한 시리즈명이 있으면 그 이름만 사용하고, 없으면 추측하지 말라.
- 응답 끝에 JSON이나 특수 포맷 쓰지 말고 순수 자연어로만`
    ...
    const userPrompt = conversationCtx
      ? `${conversationCtx}\n\n═══ 사용자 새 메시지 ═══\n${baseUserPrompt}`
      : baseUserPrompt
    const complexity = assessComplexity(clean)
    const isDeep = complexity.level === "deep"
    const deepCoTBlock = isDeep
      ? `

═══ 깊이 있는 답변 모드 (이 질문은 복잡합니다) ═══
최종 출력은 평문 3~6문장만 내보내되, 내부적으로 단계별로 생각하라 (생각 과정 나열 금지):
  1) 사용자 의도 파악 (비교/트러블슈팅/용어설명/복합조건?)
  2) 관련 도메인 지식 정리 (내열온도/헬릭스각/코팅 메커니즘/재질 특성)
  3) 최적 답변 구성 (메커니즘 → 결론 → 실전 팁 순서)
  4) 위험 요소 체크 (가공경화·채터·구성인선·열변형·흑연화 등)
시간이 걸려도 좋으니 "상황에 따라 다릅니다" 같은 shallow 답변은 절대 금지.`
      : ""
    const finalSystemPrompt = systemPrompt + deepCoTBlock
    const chatMaxTokens = isDeep ? 2000 : complexity.level === "light" ? 500 : 1500
    const raw = await provider.complete(finalSystemPrompt, [
      { role: "user", content: userPrompt }
    ], chatMaxTokens)
