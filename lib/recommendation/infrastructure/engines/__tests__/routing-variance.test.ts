...
// ---------------------------------------------------------------------------
// 8. Cross-field revision: pending question 필드 ≠ 사용자가 변경하려는 필드
//    "Ball로 바꿔주세요" while pending=coating → toolSubtype 리비전이어야 함
// ---------------------------------------------------------------------------
describe("cross-field revision (pending field ≠ target field)", () => {
  ...
  const fluteOptions = [
    { index: 1, label: "2날 (100개)", field: "fluteCount", value: "2", count: 100 },
    { index: 2, label: "4날 (200개)", field: "fluteCount", value: "4", count: 200 },
    { index: 3, label: "상관없음", field: "fluteCount", value: "skip", count: 0 },
  ]

  it('"Ball로 바꿔주세요" while pending=coating → unresolved (not mistaken as coating answer)', () => {
    const state = makeState({
      lastAskedField: "coating",
      displayedOptions: coatingOptions,
      appliedFilters: [
        { field: "toolSubtype", op: "eq" as const, value: "Square", appliedAt: 1 },
      ],
    })
    const result = resolvePendingQuestionReply(state, "Ball로 바꿔주세요")
    expect(result.kind).toBe("unresolved")
  })

  it('"Square로 변경해줘" while pending=fluteCount → unresolved (toolSubtype revision)', () => {
    const state = makeState({
      lastAskedField: "fluteCount",
      displayedOptions: fluteOptions,
      appliedFilters: [
        { field: "toolSubtype", op: "eq" as const, value: "Ball", appliedAt: 1 },
      ],
    })
    const result = resolvePendingQuestionReply(state, "Square로 변경해줘")
    expect(result.kind).toBe("unresolved")
  })

  it('"4날 대신 2날로" while pending=coating → unresolved (fluteCount revision)', () => {
    const state = makeState({
      lastAskedField: "coating",
      displayedOptions: coatingOptions,
      appliedFilters: [
        { field: "fluteCount", op: "eq" as const, value: "4", appliedAt: 1 },
      ],
    })
    const result = resolvePendingQuestionReply(state, "4날 대신 2날로")
    expect(result.kind).toBe("unresolved")
  })

  it('"직경 8mm로 바꿔줘" while pending=coating → unresolved (diameterMm revision)', () => {
    const state = makeState({
      lastAskedField: "coating",
      displayedOptions: coatingOptions,
      appliedFilters: [
        { field: "diameterMm", op: "eq" as const, value: "10", appliedAt: 1 },
      ],
    })
    const result = resolvePendingQuestionReply(state, "직경 8mm로 바꿔줘")
    expect(result.kind).toBe("unresolved")
  })

  it('"Radius 말고 Ball로 바꿔" while pending=fluteCount → unresolved (toolSubtype revision)', () => {
    const state = makeState({
      lastAskedField: "fluteCount",
      displayedOptions: fluteOptions,
      appliedFilters: [
        { field: "toolSubtype", op: "eq" as const, value: "Radius", appliedAt: 1 },
      ],
    })
    const result = resolvePendingQuestionReply(state, "Radius 말고 Ball로 바꿔")
    expect(result.kind).toBe("unresolved")
  })
})
