...
function makePrevState(): ExplorationSessionState {
  return {
    ...
    displayedChips: ["DLC (1ê°?", "TiAlN (1ê°?", "?ê??†ìŒ"],
    displayedOptions: [
      { index: 1, label: "DLC (1ê°?", field: "coating", value: "DLC", count: 1 },
      { index: 2, label: "TiAlN (1ê°?", field: "coating", value: "TiAlN", count: 1 },
    ],
...
const orchResult: OrchestratorResult = {
  action: { type: "explain_product", target: "DLCê°€ ë­ì•¼?" },
  reasoning: "test",
  agentsInvoked: [],
...
describe("handleServeGeneralChatAction", () => {
  it("preserves question assist mode for pending-field explanations", async () => {
    ...
    const response = await handleServeGeneralChatAction({
      deps: {
        ...
        handleContextualNarrowingQuestion: vi.fn(async () => "DLC???Œë£¨ë¯¸ëŠ„ ê°€ê³µì—???ì£¼ ?°ëŠ” ì½”íŒ…?…ë‹ˆ??"),
      ...
      form,
      messages: [
        { role: "ai", text: "ì½”íŒ…??? íƒ?´ì£¼?¸ìš”." },
        { role: "user", text: "DLCê°€ ë­ì•¼?" },
      ],
      prevState,
      filters,
      narrowingHistory,
  ...
  it("replaces stale narrowing chips with handler chips for inventory replies", async () => {
    const prevState = makePrevState()
    const candidates = [makeCandidate(1, "E5D7004010", "DLC"), makeCandidate(2, "E5D7004020", "TiAlN")]
    const inventoryChips = ["?¤ë¥¸ ?œí’ˆ ?¬ê³ ", "ì¶”ì²œ ?œí’ˆ ë³´ê¸°", "ì²˜ìŒë¶€???¤ì‹œ"]

    const response = await handleServeGeneralChatAction({
      deps: {
        buildCandidateSnapshot: () => prevState.displayedCandidates,
        handleDirectInventoryQuestion: vi.fn(async () => ({
          text: "E5D7004010???¬ê³  ?°ì´?°ì…?ˆë‹¤.",
      ...
      messages: [
        { role: "ai", text: "ì½”íŒ…??? íƒ?´ì£¼?¸ìš”." },
        { role: "user", text: "E5D7004010 ?¬ê³  ?Œë ¤ì¤? },
      ],
      prevState,
      filters: [],
      narrowingHistory: [],
    ...
    expect(body.chips).not.toContain("DLC (1ê°?")
    expect(body.chips).not.toContain("TiAlN (1ê°?")
  ...
  it("replaces stale narrowing chips with handler chips for brand reference replies", async () => {
    ...
    const brandChips = ["HRC ì¡°ê±´ ì¶”ê?", "?¤ë¥¸ ISO ë³´ê¸°", "ì¶”ì²œ ?œí’ˆ ë³´ê¸°"]

    const response = await handleServeGeneralChatAction({
      deps: {
        buildCandidateSnapshot: () => prevState.displayedCandidates,
        handleDirectInventoryQuestion: vi.fn(async () => null),
        handleDirectEntityProfileQuestion: vi.fn(async () => null),`r`n        handleDirectBrandReferenceQuestion: vi.fn(async () => ({
          text: "reference brand ê¸°ì??œì…?ˆë‹¤.",
          chips: brandChips,
        })),
        handleDirectCuttingConditionQuestion: vi.fn(async () => null),
        handleContextualNarrowingQuestion: vi.fn(async () => null),
      ...
      messages: [
        { role: "ai", text: "ì½”íŒ…??? íƒ?´ì£¼?¸ìš”." },
        { role: "user", text: "ISO H ë¸Œëœ??ê¸°ì???ë³´ì—¬ì¤? },
      ],
      prevState,
      filters: [],
      narrowingHistory: [],
  ...
  it("replaces stale narrowing chips with handler chips for cutting condition replies", async () => {
    const prevState = makePrevState()
    const candidates = [makeCandidate(1, "E5D7004010", "DLC")]
    const cuttingChips = ["?¤ë¥¸ ?Œì¬ ì¡°ê±´??ë³´ì—¬ì¤?, "ì¶”ì²œ ?œí’ˆ ë³´ê¸°", "ì²˜ìŒë¶€???¤ì‹œ"]
    ...
    const response = await handleServeGeneralChatAction({
      ...
      messages: [
        { role: "ai", text: "ì½”íŒ…??? íƒ?´ì£¼?¸ìš”." },
        { role: "user", text: "E5D7004010 ?ˆì‚­ì¡°ê±´ ?Œë ¤ì¤? },
      ],
      prevState,
      filters: [],
      narrowingHistory: [],
...
describe("buildReplyDisplayedOptions", () => {
  it("converts chip labels to _action DisplayedOptions", () => {
    const chips = ["?¤ë¥¸ ?œí’ˆ ?¬ê³ ", "ì¶”ì²œ ?œí’ˆ ë³´ê¸°", "ì²˜ìŒë¶€???¤ì‹œ"]
    const options = buildReplyDisplayedOptions(chips)

    expect(options).toHaveLength(3)
    expect(options[0]).toEqual({ index: 1, label: "?¤ë¥¸ ?œí’ˆ ?¬ê³ ", field: "_action", value: "?¤ë¥¸ ?œí’ˆ ?¬ê³ ", count: 0 })
    expect(options[1]).toEqual({ index: 2, label: "ì¶”ì²œ ?œí’ˆ ë³´ê¸°", field: "_action", value: "ì¶”ì²œ ?œí’ˆ ë³´ê¸°", count: 0 })
    expect(options[2]).toEqual({ index: 3, label: "ì²˜ìŒë¶€???¤ì‹œ", field: "_action", value: "ì²˜ìŒë¶€???¤ì‹œ", count: 0 })
  })
...
describe("side question suspend/resume", () => {
  it("restores suspended flow options and chips after side question answer", async () => {
    ...
    prevState.suspendedFlow = {
      ...
      pendingQuestion: "ì½”íŒ…??? íƒ?´ì£¼?¸ìš”.",
      displayedOptionsSnapshot: [
        { index: 1, label: "DLC (1ê°?", field: "coating", value: "DLC", count: 1 },
        { index: 2, label: "TiAlN (1ê°?", field: "coating", value: "TiAlN", count: 1 },
      ],
      displayedChipsSnapshot: ["DLC (1ê°?", "TiAlN (1ê°?", "?ê??†ìŒ"],
    ...
    const response = await handleServeGeneralChatAction({
      deps: {
        ...
        handleGeneralChat: vi.fn(async () => ({
          text: "?µì‚° ê³µì¥?€ ì¶©ë‚¨???„ì¹˜???ˆìŠµ?ˆë‹¤.",
          chips: ["ì²˜ìŒë¶€???¤ì‹œ"],
        })),
        jsonRecommendationResponse: (params) =>
          new Response(JSON.stringify(params), { headers: { "content-type": "application/json" } }),
      },
      action: { type: "answer_general", message: "?µì‚° ê³µì¥ ?•ë³´ì¤? } as any,
      orchResult: { ...orchResult, action: { type: "answer_general" as const, message: "?µì‚° ê³µì¥ ?•ë³´ì¤? } },
      provider: { available: () => false } as any,
      form,
      messages: [
        { role: "ai", text: "ì½”íŒ…??? íƒ?´ì£¼?¸ìš”." },
        { role: "user", text: "?µì‚° ê³µì¥ ?•ë³´ì¤? },
      ],
      prevState,
      filters: [],
      narrowingHistory: [],
    ...
    expect(body.chips).toEqual(["DLC (1ê°?", "TiAlN (1ê°?", "?ê??†ìŒ"])
    ...
    expect(body.text).toContain("?¤ì‹œ ?œí’ˆ ì¶”ì²œ?¼ë¡œ ?Œì•„ê°ˆê²Œ??)
  ...
  it("does NOT trigger suspend for question-assist (DLCê°€ ë­ì•¼?)", async () => {
    const prevState = makePrevState()
    ...
    expect(body.text).not.toContain("?¤ì‹œ ?œí’ˆ ì¶”ì²œ?¼ë¡œ ?Œì•„ê°ˆê²Œ??)
  ...
  it("restores pendingField on resume even through inventory reply path", async () => {
    ...
    prevState.suspendedFlow = {
      ...
      displayedChipsSnapshot: ["DLC (1ê°?", "?ê??†ìŒ"],
      reason: "side_question",
    }
    const candidates = [makeCandidate(1, "E5D7004010", "DLC")]
    const inventoryChips = ["?¤ë¥¸ ?œí’ˆ ?¬ê³ ", "ì¶”ì²œ ?œí’ˆ ë³´ê¸°"]

    const response = await handleServeGeneralChatAction({
      deps: {
        buildCandidateSnapshot: () => prevState.displayedCandidates,
        handleDirectInventoryQuestion: vi.fn(async () => ({
          text: "E5D7004010???¬ê³ ?…ë‹ˆ??",
          chips: inventoryChips,
        })),
        handleDirectEntityProfileQuestion: vi.fn(async () => null),`r`n        handleDirectBrandReferenceQuestion: vi.fn(async () => null),
        handleDirectCuttingConditionQuestion: vi.fn(async () => null),
    ...
    expect(body.chips).toEqual(["DLC (1ê°?", "?ê??†ìŒ"])
    expect(body.sessionState.lastAskedField).toBe("coating")
    expect(body.text).toContain("?¤ì‹œ ?œí’ˆ ì¶”ì²œ?¼ë¡œ ?Œì•„ê°ˆê²Œ??)
    expect(body.sessionState.suspendedFlow).toBeNull()
  })
