...
function makePrevState(): ExplorationSessionState {
  ...
  return {
    ...
    displayedChips: ["?°ì§ˆ", "ì£¼ì¡° ?©ê¸ˆ", "?¨ì¡° ?©ê¸ˆ", "?ê??†ìŒ"],
    displayedOptions: [
      { index: 1, label: "?°ì§ˆ", field: "workPieceName", value: "?°ì§ˆ", count: 0 },
      { index: 2, label: "ì£¼ì¡° ?©ê¸ˆ", field: "workPieceName", value: "ì£¼ì¡° ?©ê¸ˆ", count: 0 },
    ],
...
const form: ProductIntakeForm = {
  material: { status: "known", value: "?Œë£¨ë¯¸ëŠ„" },
...
describe("serve engine general chat company replies", () => {
  it("answers office questions from deterministic knowledge and preserves the pending recommendation UI", async () => {
    ...
    const response = await handleServeGeneralChatAction({
      ...
      action: { type: "explain_product", target: "ë¶€?°ì˜?…ì†Œ???€?´ì„œ ?Œë ¤ì¤? },
      orchResult: {
        action: { type: "explain_product", target: "ë¶€?°ì˜?…ì†Œ???€?´ì„œ ?Œë ¤ì¤? },
        reasoning: "test",
        agentsInvoked: [],
      ...
      form,
      messages: [
        { role: "ai", text: "?Œë£¨ë¯¸ëŠ„ ?¸ë? ì¢…ë¥˜ë¥??Œë ¤ì£¼ì‹œê² ì–´??" },
        { role: "user", text: "ë¶€?°ì˜?…ì†Œ???€?´ì„œ ?Œë ¤ì¤? },
      ],
      prevState,
      filters: [],
      narrowingHistory: [],
    ...
    expect(body.text).toContain("ë¶€?°ì˜?…ì†Œ")
