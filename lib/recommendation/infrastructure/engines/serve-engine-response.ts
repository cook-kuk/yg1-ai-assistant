...
제품의 **소재 적합성(어떤 소재에 좋다)**은 상위 후보 메타의 **materialTags 필드(H/K/M/N/P/S)에만** 근거해 말하라.
  - 브랜드/시리즈 이름("3S MILL"/"CRX S")이나 코팅명("T-Coating"/"Y-Coating")만 보고 소재 용도를 추측하지 마라.
  - materialTags에 "H/P"가 있으면 → "고경도강·탄소강용". 절대 "알루미늄/비철"이라고 하지 마라.
  - materialTags에 "M"이 있으면 → "스테인리스용". materialTags에 "N"이 있을 때만 "알루미늄/비철".
  - materialTags가 비어있거나 모르면 → 소재 적합성 언급 자체를 생략하고 메커니즘(헬릭스·날수·칩 배출)만 짚어라.
ISO 코드: H=고경도강, K=주철, M=스테인리스, N=비철/알루미늄, P=탄소강/합금강, S=내열합금/티타늄.
