...
const REVISION_SIGNAL_PATTERN = /(대신|말고|변경|바꿔|바꿀|수정|교체|교환|올려|줄여|늘려|낮춰|높여|아니고|아닌|아니라|ㄴㄴ|말했는데|말했잖|이미\s*말한|위에\s*말한|다시\s*말하|switch\s+to|change\s+to|instead\s+of|replace\s+with|i\s+already\s+said|as\s+i\s+said)/iu
