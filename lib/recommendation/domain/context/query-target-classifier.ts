/**
 * Query Target Classifier
 *
 * Separates the user's answer topic from the current narrowing field.
 * Active filters are constraints, not automatically the topic.
 */
...
const COMPARISON_PATTERN = /차이|비교|vs|대비|어떤.*게.*더|뭐가.*다른|뭐가.*좋고|어떤.*게.*좋/i
...
const FIELD_KEYWORDS: Record<string, RegExp> = {
  ...
  fluteCount: /날수|플루트|flute|\d날/i,
  toolSubtype: /형상|서브타입|Square|Ball|Radius|Taper|스퀘어|볼|래디우스/i,
  material: /소재|재질|피삭재|알루미늄|스테인리스|탄소강/i,
  diameterMm: /직경|지름|mm/i,
  toolMaterial: /공구\s*재질|카바이드|초경|HSS|고속도강/i,
}
...
export function classifyQueryTarget(
  ...
  const comparisonWithConjunction = clean.match(/(.+?)\s*[과와랑]\s*(.+?)\s*(차이|비교|다른|좋|별로|뭐)/i)
