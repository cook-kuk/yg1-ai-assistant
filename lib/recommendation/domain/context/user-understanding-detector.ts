...
export function detectUserState(
  ...
  if (!userMessage) return { state: "clear", confidence: 0.5, confusedAbout: null }

  const clean = userMessage.trim().toLowerCase()

  // Delegation
  if (DELEGATION_PATTERNS.some(p => p.test(clean))) {
    return { state: "wants_delegation", confidence: 0.9, confusedAbout: null }
  }

  // Explicit explanation request
  if (EXPLANATION_PATTERNS.some(p => p.test(clean))) {
    // Try to extract what they're confused about
  ...
  if (CONFUSION_PATTERNS.some(p => p.test(clean))) {
    ...
    return {
      state: "confused",
      confidence: 0.85,
      confusedAbout: aboutMatch ? aboutMatch[1].trim() : pendingQuestionField ?? null,
    }
  }

  // Skip
  if (SKIP_PATTERNS.some(p => p.test(clean))) {
    return { state: "wants_skip", confidence: 0.85, confusedAbout: null }
  }

  // Revision
  if (REVISION_PATTERNS.some(p => p.test(clean))) {
    return { state: "wants_revision", confidence: 0.8, confusedAbout: null }
  }

  // Uncertain (short message with question marks but no clear intent)
  if (clean.length < 15 && /[?？]/.test(clean) && !/\d/.test(clean)) {
    return { state: "uncertain", confidence: 0.6, confusedAbout: null }
  }

  return { state: "clear", confidence: 0.7, confusedAbout: null }
}
